...
def scopusReport():
    ...
    if form.validate_on_submit():
        ...
        if my_sc.sc_rep_article:
            ...
            sc_exporter:ScopusExportExcel = ScopusExportExcel()
            list_export=wos_dbase.select_authors_by_form(form)
            fm=f"wos_report_author_{date.today()}.xlsx"
            return Response(sc_exporter.create_report_authors_with_stat(list_export,wos=True),
                            headers={'Content-Disposition': f'attachment; filename={fm}',
                                     'Content-type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})   
